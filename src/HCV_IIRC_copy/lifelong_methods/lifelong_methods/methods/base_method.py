...
class BaseMethod(ABC, nn.Module):
    ...
    def __init__(self, n_cla_per_tsk: Union[np.ndarray, List[int]], class_names_to_idx: Dict[str, int],tasks, config: Dict):
        ...
        self.num_classes = int(len(class_names_to_idx))
    ...
    def step_scheduler(self, val_metric: Optional = None) -> None:
        ...
        if self.reduce_lr_on_plateau:
            ...
            try:
                self.scheduler_exp_opt.step(val_metric)
            except:
                pass
        else:
            self.scheduler.step()
            try:
                self.scheduler_exp_opt.step()
            except:
                pass
    ...
    def _compute_offsets(self, task) -> Tuple[int, int]:
        ...
        # repeat classes won't work like this, you'll have to output the entire layer coz if you have ABCDE in seen classes and need AE in next task, you'll have to consider first and last logit which isn't possible
        # offset1 = len(self.seen_classes) - len(list(set(self.seen_classes) & set(self.tasks[task])))
        # offset2 = len(self.seen_classes)
