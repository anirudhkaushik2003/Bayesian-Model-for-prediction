...
from experiments.train_bayesian import task_train, tasks_eval
...
def main_worker(gpu, config: dict, dist_args: dict = None):
    ...
    # for cur_task_id in range(starting_task, 2):
        ...
        cur_task_classes = [metadata["class_names_to_idx"][i] for i in tasks[cur_task_id]]
        ...
        model.consolidate_task_knowledge( train_loader, config, metadata, cur_task_classes, gpu,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tasks_configuration_id', type=int, default=4, choices=range(0, 10),
                        help="The task configuration id. Ignore for joint training")
    # The training method  ###############################################
    parser.add_argument('--method', type=str, default="bayesian_prediction",
                        choices=["finetune", "mask_seen_classes", "lucir", "lucir_noloss3","agem", "icarl_cnn",
                                 "icarl_norm", "learning_without_forgetting", "bayesian_prediction"])
    ################################################################################
