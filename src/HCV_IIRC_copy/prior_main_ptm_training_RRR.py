...
from experiments.ptm_train_RRR import task_train, tasks_eval, task_eval_superclass_in_training
...
def main_worker(gpu, config: dict, dist_args: dict = None):
    ...
    model = method.Model(metadata["n_cla_per_tsk"], metadata["class_names_to_idx"],tasks, config)
    ...
    # RRR Specific
    saliency_config = copy.deepcopy(config)
    saliency_config["n_memories_per_class"] = saliency_config["saliency_n_memories_per_class"]
    saliency_config["n_memories_per_class"] = saliency_config["saliency_total_n_memories"]

    saliency_buffer = method.Buffer(saliency_config, buffer_dir, map_size, essential_transforms_fn, augmentation_transforms_fn)
    ...
    NUM_CLASS_PER_TASK=2 # remember to change accordingly
    ...
    # for cur_task_id in range(starting_task, 4):
        ...
        if cur_task_id > 1:
            ...
            for ind in range(len(prev_verified_super_key)):
                prev_verified_super_key[ind] = np.append(prev_verified_super_key[ind], [0]*NUM_CLASS_PER_TASK)
        ...
        task_train(model, buffer, saliency_buffer, lifelong_datasets, config, metadata, logbook=logbook, dist_args=dist_args,
        ...
        saliency_buffer.update_buffer_new_task(
            lifelong_datasets["train"], model, dist_args=dist_args, model=model, batch_size=config["batch_size"]
        )
        ...
        if distributed and len(saliency_buffer) > 0:
            with saliency_buffer.disable_augmentations():
                image_1, _,  _, _ = saliency_buffer[-1]
                image_1 = image_1.to(device)
                if rank == 0:
                    image_2, _, _, _ = saliency_buffer[-1]
                    image_2 = image_2.to(device)
                else:
                    image_2 = torch.empty_like(image_1)
                torch.distributed.broadcast(image_2, 0)
...
if __name__ == '__main__':
    ...
    # Parameters for creating the tasks # change range from 0 to 10 if more than 10 tasks
    ...
    # The training method  ###############################################
    parser.add_argument('--method', type=str, default="learning_without_forgetting_ptm_RRR",choices=[ "learning_without_forgetting_ptm_RRR"])
    ################################################################################
    ...
    ### RRR Specific Begin
    parser.add_argument('--saliency_total_n_memories', type=int, default=-1,
                        help="The total replay buffer size, which is divided by the observed number of classes to get "
                             "the number of memories kept per task, note that the number of memories per task here is "
                             "not fixed but rather decreases as the number of tasks observed increases (with a minimum "
                             "of 1). If n_memories_per_class is set to a value greater than -1, the "
                             "n_memories_per_class is used instead.")
    parser.add_argument('--saliency_n_memories_per_class', type=int, default=20,
                        help="The number of samples to keep from each class, if set to -1, the total_n_memories "
                             "argument is used instead")
