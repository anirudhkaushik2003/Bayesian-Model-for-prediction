...
def log(epoch, task_id, log_dict, logbook):
    ...
    log_dict["step"] = epoch
    images = None
    if f"log_images_{task_id}" in log_dict.keys():
        images = log_dict.pop(f"log_images_{task_id}")
    logbook.write_metric(log_dict)
    if images is not None:
        wandb.log({f"Visualization_{task_id}": [wandb.Image(images[0], caption="Original"), wandb.Image(images[1], caption="GradCAM"),wandb.Image(images[2], caption="Heatmap") ]},step=epoch)
...
def log_task(task_id, log_dict, logbook):
    ...
    logbook.write_metric(log_dict)
    if images is not None:
        wandb.log({f"Visualization_task_{task_id}": [wandb.Image(images[0], caption="Original"), wandb.Image(images[1], caption="GradCAM"),wandb.Image(images[2], caption="Heatmap") ]})
