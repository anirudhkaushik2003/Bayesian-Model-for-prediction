...
def evaluate(model, dataloader, config, metadata, test_mode=False, gpu=None,verified_super_key=None,super_class_index=None,sub_class_index=None, task_end=False):
    valid_loss = 0
    # valid_metrics = {'jaccard_sim': 0., 'modified_jaccard': 0., 'strict_acc': 0., 'recall': 0.}
    valid_metrics = {'strict_acc': 0.}
    # valid_metrics = {'strict_acc': 0.,'mean_class_recall': 0.}
 ...
 #########################################
    with torch.no_grad():
        for minibatch in dataloader:
            ...
            if not test_mode:
                predictions, loss = model.observe(images, labels, in_buffer, train=False, task_end=True)
            ...
            # valid_metrics['mean_class_recall'] += metrics.recall(predictions, labels) * images.shape[0]
            data_len += images.shape[0]
        valid_loss /= data_len
        # valid_metrics['jaccard_sim'] /= data_len
        # valid_metrics['modified_jaccard'] /= data_len
        valid_metrics['strict_acc'] /= data_len
        # valid_metrics['mean_class_recall'] /= data_len
...
def task_train(model, buffer, lifelong_datasets, config, metadata, logbook, dist_args=None,
    ...
    for epoch in range(config['task_epoch'], num_epochs):
        ...
        valid_loss, valid_metrics = evaluate(model, valid_loader, config, metadata, test_mode=False, gpu=gpu,
                                             ...
                                             sub_class_index=sub_class_index,task_end=False)
...
def tasks_eval(model, dataset, cur_task_id, config, metadata, logbook, dataset_type="valid", dist_args=None, verified_super_key=None):
    ...
    # for task_id in range(1,cur_task_id + 1):
        ...
        _, metrics = evaluate(model, dataloader, config, metadata, test_mode=True, gpu=gpu, task_end=True)
    ...
    _, metrics = evaluate(model, dataloader, config, metadata, test_mode=True, gpu=gpu,verified_super_key=verified_super_key, task_end=True)
