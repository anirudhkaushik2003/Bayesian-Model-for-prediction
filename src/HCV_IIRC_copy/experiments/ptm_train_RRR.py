...
from lifelong_methods.buffer.buffer import SaliencyBuffer
...
def epoch_train(model, dataloader, saliency_loader, config, metadata, gpu=None, rank=0,verified_super_key_dict=None,super_class_index=None,sub_class_index=None):
    train_loss = 0
    ...
    # RRR
    if model.cur_task_id > 0:
        minibatch_i = 0
        for minibatch in saliency_loader:
            labels_names = list(zip(minibatch[2], minibatch[3]))
            ...
            if gpu is None:
                ...
                saliency = minibatch[1].to(config["device"], non_blocking=True)
                labels = labels.to(config["device"], non_blocking=True)
            else:
                images = minibatch[0].to(torch.device(f"cuda:{gpu}"), non_blocking=True)
                saliency = minibatch[1].to(torch.device(f"cuda:{gpu}"), non_blocking=True)
                labels = labels.to(torch.device(f"cuda:{gpu}"), non_blocking=True)

            if len(minibatch) > 4:
                if gpu is None:
                    in_buffer = minibatch[4].to(config["device"], non_blocking=True)
                else:
                    in_buffer = minibatch[4].to(torch.device(f"cuda:{gpu}"), non_blocking=True)
            else:
                in_buffer = None

            ###########################################################
            model.observe_sal(images, saliency, labels, in_buffer, train=True) # only adjusts model params
...
def evaluate(model, dataloader, config, metadata, test_mode=False, gpu=None,verified_super_key_dict=None,super_class_index=None,sub_class_index=None):
    ...
    for minibatch in dataloader:
        ...
        random_index = np.random.randint(0, len(minibatch[0]) - 1)
        test_image = images[random_index].unsqueeze(0)
        test_label = labels[random_index]
        break 

    target_layers = [model.net.model.layer3[-1]]
    cam = GradCAM(model=model.net.model, target_layers=target_layers)
    grayscale_cam = cam(input_tensor=test_image,)
    grayscale_cam = grayscale_cam[0, :]
    # heatmap = cv2.applyColorMap(np.uint8(grayscale_cam), cv2.COLORMAP_JET)
    # # heatmap = np.uint8(255 * heatmap)
    img_to_show = np.transpose(test_image.squeeze().cpu().detach().numpy(), (1,2,0))
    img_to_show = cv2.normalize(img_to_show, None, 0, 0.999, cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    # superimposed_img = cv2.addWeighted(heatmap, 0.5, img_to_show, 0.5, 0)
    superimposed_img = show_cam_on_image(img_to_show, grayscale_cam, use_rgb=True)

    log_images  = [img_to_show, superimposed_img, grayscale_cam]

    return valid_loss, valid_metrics, log_images


def task_train(model, buffer,saliency_buffer, lifelong_datasets, config, metadata, logbook, dist_args=None,
    ...
    if config["method"] == "agem":
        ...
        task_train_data_with_buffer = TaskDataMergedWithBuffer(buffer, task_train_data, buffer_sampling_multiplier=bsm) # here
        saliency_maps = SaliencyBuffer(saliency_buffer, task_train_data, buffer_sampling_multiplier=bsm)
    ...
    saliency_loader = data.DataLoader(
        saliency_maps, batch_size=config["batch_size"], shuffle=(train_sampler is None),
        num_workers=config["num_workers"], pin_memory=True, sampler=train_sampler
    )
    ...
    for epoch in range(config['task_epoch'], num_epochs):
        ...
        log_dict = {}
        train_loss, train_metrics = epoch_train(model, train_loader, saliency_loader,  config, metadata, gpu, rank,
                                                verified_super_key_dict=verified_super_key_dict,
                                                super_class_index=super_class_index,
                                                sub_class_index=sub_class_index)
        ...
        valid_loss, valid_metrics, log_images = evaluate(model, valid_loader, config, metadata,
        ...
        log_dict[f"log_images_{cur_task_id}"] = log_images
...
def tasks_eval(model, dataset, cur_task_id, config, metadata, logbook, dataset_type="valid", dist_args=None, verified_super_key_dict=None):
    ...
    # for task_id in range(1,cur_task_id + 1):
        ...
        _, metrics, log_images = evaluate(model, dataloader, config, metadata, test_mode=True, gpu=gpu,
                              verified_super_key_dict=verified_super_key_dict)
        metrics_dict[f"log_images_{cur_task_id}"] = log_images
...
def task_eval_superclass(model, dataset, cur_task_id, config, metadata, logbook,
                         dataset_type="valid", dist_args=None,Threshold=0.58, num_class_per_task=8):
    """log the accuracies of the new model on all observed tasks
    :param metadata:
    """
    assert dataset.complete_information_mode is True
...
def task_eval_superclass_in_training(model, dataset, cur_task_id, config, metadata, logbook,
                         dataset_type="valid", dist_args=None,Threshold=0.6, num_class_per_task=8):
    """log the accuracies of the new model on all observed tasks
    :param metadata:
    """
    # assert dataset.complete_information_mode is True
...
def eval_superclass(model, dataloader, config, metadata, test_mode=False, gpu=None, Threshold=0.58, num_class_per_task=8):
    valid_loss = 0
    valid_metrics = {'jaccard_sim': 0., 'modified_jaccard': 0., 'strict_acc': 0., 'recall': 0.}
    data_len = 0
    class_names_to_idx = metadata["class_names_to_idx"]
...
def eval_superclass_in_training(model, dataloader, config, metadata, test_mode=False, gpu=None, Threshold=0.58, num_class_per_task=8):
    valid_loss = 0
    valid_metrics = {'jaccard_sim': 0., 'modified_jaccard': 0., 'strict_acc': 0., 'recall': 0.}
    data_len = 0
    class_names_to_idx = metadata["class_names_to_idx"]
